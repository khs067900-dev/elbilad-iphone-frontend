...
function IPhone18ProCard({ product, index }: { product: Product; index: number }) {
  ...
  return (
    <div
      className="i18p-card group cursor-pointer"
      style={{ animationDelay: `${0.05 * index}s` }}
      dir="rtl"
      onClick={() => router.push(`/product/${product._id}`)}
    >
      {/* image */}
      <div className="i18p-img-wrap">
        ...
        <div
          className={`absolute top-2 left-2 flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-xl shadow-md ${
            product.inStock ? "bg-emerald-500 text-white" : "bg-gray-400 text-white"
          }`}
        >
          <Icon
            icon={product.inStock ? "solar:check-circle-bold" : "solar:close-circle-bold"}
            width={11}
          />
          ...
          <div className="flex flex-wrap gap-1">
            {[
              ...
              .map((s: any) => (
                <span
                  key={s.label}
                  className="flex items-center gap-1 text-[9px] sm:text-[10px] font-semibold text-[#155E6F] bg-[#155E6F]/8 border border-[#155E6F]/15 px-1.5 py-0.5 rounded-lg"
                >
          ...
          <div className="flex items-center gap-1.5 bg-[#6DBE00]/10 border border-[#6DBE00]/20 rounded-xl px-2 py-1">
            ...
            <span className="text-[9px] sm:text-[10px] font-bold text-[#4a8a00]">
              ...
              {product.installment.downPayment
                ? `من ${fmt(product.installment.downPayment)} ر.س`
                : "متاح"}
            </span>
          </div>
        )}
...
export default function IPhone18ProClient({ products }: { products: Product[] }) {
  ...
  return (
    <>
      <style>{`
        @keyframes i18pSlideUp { from{opacity:0;transform:translateY(20px)} to{opacity:1;transform:translateY(0)} }
        @keyframes i18pCardReveal { from{opacity:0;transform:translateY(24px) scale(.97)} to{opacity:1;transform:translateY(0) scale(1)} }
        ...
        .i18p-card {
          ...
          animation: i18pCardReveal .45s ease forwards;
          opacity:0;
        ...
        .i18p-slide-up { animation: i18pSlideUp .5s ease forwards; }
        .i18p-slide-up-1 { animation: i18pSlideUp .5s .1s ease both; }
        .i18p-slide-up-2 { animation: i18pSlideUp .5s .2s ease both; }
        ...
        .cart-btn-v2:hover .cart-btn-v2-bg { transform: scale(1.05); }
        .cart-btn-v2:active .cart-btn-v2-bg { transform: scale(0.98); }
      `}</style>
      ...
      <main className="min-h-screen" dir="rtl" style={{ background: "#f5f7f9" }}>
        ...
        <div className="relative overflow-hidden" style={{ height: 400 }}>
          <Image
            src="https://res.cloudinary.com/dllmx2yf3/image/upload/v1789347926/b23c542a-4cfb-400d-a99d-f02ed9b882d9_dfunoo.webp"
            alt="iPhone 18 Pro Hero"
            fill
          ...
          <div className="relative z-10 h-full flex flex-col justify-between max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
            {/* breadcrumb */}
            <div className="i18p-slide-up flex items-center gap-1.5 text-xs sm:text-sm">
              ...
              <IoChevronBack size={11} className="text-white/30" />
              <span className="text-white font-semibold">iPhone 18 Pro</span>
            </div>

            {/* title */}
            <div className="i18p-slide-up-1 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
              <div>
              ...
              <div className="i18p-slide-up-2 flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-2xl px-4 py-2.5 border border-white/20 self-start sm:self-auto">
            ...
            <>
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-5">
                {currentProducts.map((p, i) => (
                  <IPhone18ProCard key={p._id} product={p} index={i} />
                ))}
              </div>
