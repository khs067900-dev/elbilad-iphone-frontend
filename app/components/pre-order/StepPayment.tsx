...
export function StepPayment({
  ...
  useEffect(() => {
    if (!selectedPayment) setSelectedPayment("mada");
  }, [selectedPayment]);
          ...
          {(["mada", "mastercard"] as const).map(method => (
            <button key={method} onClick={() => setSelectedPayment(method)}
              ...
              <div className="w-10 h-5 sm:w-16 sm:h-9 relative shrink-0">
                <Image src={method === "mada" ? "/mada975b.png" : "/cc975b copy.png"} alt={method} fill className="object-contain" />
              </div>
            </button>
          ))}
        ...
        {selectedPayment && selectedPayment !== "applepay" && (
          <div className="mt-5 space-y-4">
            ...
            <button onClick={handleCardSubmit}
              disabled={cardNumber.replace(/\s/g, "").length !== 16 || cardExpiry.replace(/\D/g, "").length !== 4 || cardCvv.length !== 3 || !cardHolder.trim() || !!cardNumberError || !!cardExpiryError || loading}
              ...
              <Lock size={15} />
              {loading ? "جاري الإرسال..." : `تأكيد ودفع ${fmt(PRE_ORDER_DEPOSIT)} ريال`}
            </button>
